...
dd6 = function(S){(d2C_dK2(S, 100, 0.0386, v(100, 0.07), 0.07)*exp(0.0386*0.07))}
...
dd5 = function(S){(d2C_dK2(S, 150, 0.0386, v(150, 0.02), 0.02)*exp(0.0386*0.02))}
cunint5 = cubintegrate(dd5, lower = 50, upper = 250, method = "hcubature", maxEval = 500)
...
plot(s,t5, type = "l", xlim = c(50, 250), xlab = "Stock Price", ylab = "Value", col = "steelblue1")
abline(v=150, col = "springgreen3")
legend("topright", lwd=1,legend=c("Risk neutral density",
                                  "Strike Price"), col=c("steelblue1", "springgreen3")  ,cex = 1)
sum(c(t5*0.5))


# Pricing function for 200 points
set.seed(2)
priskt <- testset[c(sample(length(x_test_i[,1]),200)), ]

prisen = c()
for (i in 1:length(priskt[,1])) {
  CKtau <- function(S){
    pris <- (S-priskt$Strike.Price[i])*(d2C_dK2(S, priskt$Strike.Price[i], 0.0386, v(priskt$Strike.Price[i],priskt$Time.to.Maturity[i]),  priskt$Time.to.Maturity[i])*exp(0.0386*priskt$Time.to.Maturity[i]))
    return(pris)
  }
  L = cubintegrate(CKtau, lower = priskt$Strike.Price[i], upper = Inf, method = "hcubature", maxEval = 500)
  prisen = rbind(prisen, L)
}
saveRDS(prisen, file = "prisen200")

prisen = data.frame(prisen)

# Plot distribution of K and tau
par(mfrow=c(1,2))
plot(priskt$Strike.Price, col = "brown1", ylab = "Strike Price")
plot(priskt$Time.to.Maturity, col ="brown1", ylab = "Time to Maturity")

# Plot Price, Calculated Price and error
par(mfrow=c(1,1))
plot(simplify2array(priskt$Option.Price), ylab = "Price")
lines(simplify2array(prisen$integral), col = "brown1", type = "p")
legend("top", lwd=1,legend=c("Real Price",
                             "Calculated Price"), col=c("black", "brown1")  ,cex = 1, pch = 1, lty = FALSE)
plot(simplify2array(priskt$Option.Price)-simplify2array(prisen$integral), type = "l", ylab = "Error", col = "brown1")

# Mean error and mean percentage error
mean(simplify2array(priskt$Option.Price)-simplify2array(prisen$integral))
mean(((simplify2array(priskt$Option.Price)-simplify2array(prisen$integral))/simplify2array(prisen$integral))*100)

# Plot Price, Calculated Price and error with respect to strike price
par(mfrow=c(1,2))
prisen = cbind(priskt$Time.to.Maturity, prisen)
plot(priskt$Strike.Price, priskt$Option.Price, ylab = "Price", xlab = "Strike Price")
lines(priskt$Strike.Price, prisen$integral, col = "brown1", type = "p")
legend("topright", lwd=1,legend=c("Real Price",
                                  "Calculated Price"), col=c("black", "brown1")  ,cex = 1, pch = 1, lty = FALSE)
plot(priskt$Strike.Price, abs(simplify2array(priskt$Option.Price)-simplify2array(prisen$integral)), col = "red", type = "p", ylab = "Absolut Error",  xlab = "Strike Price")
