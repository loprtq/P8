...
# Get/sort data Amazon
nydata_mimp2 <- readRDS("data_w_implied_volatility_uniroot_AMZN")
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Stock Price`) >= 102)
nydata_mimp2 <- filter(nydata_mimp2, nydata_mimp2$`Time to Maturity` <= "0.1 days")
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Stock Price`) <= 104)
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Implied Volatility`) >= 0.01)

# Get/sort data Tesla
nydata_mimp2 <- readRDS("data_w_implied_volatility_uniroot_TSLA2")
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Stock Price`) >= 160)
nydata_mimp2 <- filter(nydata_mimp2, nydata_mimp2$`Time to Maturity` <= "0.1 days")
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Stock Price`) <= 162)
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Implied Volatility`) >= 0.01)

# Get/sort data Apple
nydata_mimp2 <- readRDS("data_w_implied_volatility_uniroot_AAPL")
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Stock Price`) >= 164)
nydata_mimp2 <- filter(nydata_mimp2, nydata_mimp2$`Time to Maturity` <= "0.1 days")
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Stock Price`) <= 166)
nydata_mimp2 <- filter(nydata_mimp2, as.numeric(nydata_mimp2$`Implied Volatility`) >= 0.229)


# Some of the numbers change when working with other stock than TSLA

# Plot data
plotdata <- as.data.frame(lapply(nydata_mimp2[,c(2,5,7)], as.numeric))
fig2 <- plot_ly() %>% 
  add_markers(x=plotdata$Time.to.Maturity, y=plotdata$Strike.Price, z=plotdata$Implied.Volatility, marker=list(size=3, color=plotdata$Implied.Volatility, colorscale=list(c(0, 0.1,1), c("black", "red", "pink")))) %>%
...
trainset <- as.data.frame(lapply(nydata_mimp2[,c(2,5,7)], as.numeric))[c(sample(length(nydata_mimp2[,1]),18000)), ]
testset <- as.data.frame(lapply(nydata_mimp2[-c(as.numeric(rownames(trainset))), ][, c(2,5,7)], as.numeric))
...
opt_adam = optimizer_adam(learning_rate = 0.0005, beta_1 = 0.80, beta_2 = 0.90)
...
# Saving the Network or loading, Replace TSLA with AMZN or AAPL
mymodel = load_model_hdf5("FærdigTSLA.h5")
mymodelmetrics = readRDS("FærdigTSLARDS")
...
# Derivatives of volatility surface with finite difference
delta = 1/5
Tfrom = 0
Tto = 1
Tantal = 100
...
f_mærkemærke = (plus1-2*nul+minus1)/(delta^2)
fig <- plot_ly() %>% 
